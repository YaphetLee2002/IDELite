...
public:
    IDELitePrivate(QWidget *contentWidget) : contentWidget(contentWidget) {}

    QWidget *contentWidget;
};

IDELite::IDELite(QWidget *parent) : QWidget(parent) {
    ...
    connect(screenButton->maxButton, &QPushButton::clicked, this, &IDELite::maxScreen);
    ...
    connect(startWidget->newButton, &QPushButton::clicked, this, &IDELite::newStart);
    connect(startWidget->openButton, &QPushButton::clicked, this, &IDELite::openStart);

    // 运行结果回显
    console= new ConsoleWidget();
    console->setFocusPolicy(Qt::NoFocus);
    console->setFont(runFont);
    ...
    connect(runAction, SIGNAL(triggered()), this, SLOT(on_runAction_triggered())); //运行

    QAction* compileAndRunAction = new QAction(this);
    buildMenu->addAction(compileAndRunAction);
    compileAndRunAction->setText(tr("CompileAndRun"));
    connect(compileAndRunAction, SIGNAL(triggered()), this, SLOT(on_compileAndRunAction_triggered())); //编译运行

    QAction* debugAction = new QAction(this);
    buildMenu->addAction(debugAction);
    debugAction->setText(tr("Debug"));
    connect(debugAction, SIGNAL(triggered()), this, SLOT(on_debugAction_triggered()));//debug
    ...
    d = new IDELitePrivate(contentWidget);
    ...
    connect(console, SIGNAL(sendDataDone()), this, SLOT(on_sendData()));
    connect(&dmw, SIGNAL(beginDebug()), this, SLOT(on_beginDebug()));
    connect(&dmw, SIGNAL(nextMove()), this, SLOT(on_nextMove()));
}
...
// 文件名的setter
void IDELite::setFileName(QString name)
{
    this->fileNameLabel->setText(name);
}

//编译
int IDELite::on_compileAction_triggered()
{
    this->console->clear();
    ...
    QString cmd = QString("gcc %1 -o %2").arg(demo+".c").arg(demo);
    ...
    this->console->mergeCurrentCharFormat(fmt);
    this->console->appendPlainText("正在编译...");
    ...
    if(ret!=0)
    {
        this->console->appendPlainText("编译失败...");
    }else{
        this->console->appendPlainText("编译成功...");
    }
    fmt.setForeground(QBrush("black"));
    this->console->mergeCurrentCharFormat(fmt);
    return ret!=0?0:1;
...
void IDELite::on_sendData(){
    if(cmdProcess->isOpen()){
        cmdProcess->write(codec->fromUnicode(console->sendData).data());
    }
}

void IDELite::on_readerror()
{
...
//编译并执行
void IDELite::on_compileAndRunAction_triggered()
{
    this->console->clear();
    int ret = on_compileAction_triggered();
    QString demo = fileList->currentFile;
    demo.replace(".c","");
    if(ret==0)
    {
...
// 运行
void IDELite::on_runAction_triggered(){
    this->console->clear();
    cmdProcess = new QProcess(this);//新建进程
...
// debug
void IDELite::on_debugAction_triggered(){
    dmw.show();

}

void IDELite::on_beginDebug(){
    this->console->clear();
    cmdProcess = new QProcess(this);//新建进程
    connect(cmdProcess , SIGNAL(readyReadStandardOutput()) , this , SLOT(on_readoutput()));//绑定得到输出槽函数
    connect(cmdProcess , SIGNAL(readyReadStandardError()) , this , SLOT(on_readerror()));//绑定发生错误槽函数
    ...
    QString target;
    if(dmw.breakPoint == 0){
         target = QString("gdb %1\nr\n").arg(demo).arg(dmw.breakPoint);
    }else{
         target = QString("gdb %1\nb %2\nr\n").arg(demo).arg(dmw.breakPoint);
    }
    cmdProcess->write(codec->fromUnicode(target).data());
    ...
    fmt.setForeground(QBrush("red"));
    this->console->mergeCurrentCharFormat(fmt);
    this->console->appendPlainText("开始调试");
...
void IDELite::on_nextMove(){
    QString target = QString("n\n");
    cmdProcess->write(codec->fromUnicode(target).data());
}

void IDELite::setEdit(){
...
void IDELite::setPurple()
{
    ...
    console->setStyleSheet("QPlainTextEdit{background-color:rgb(255, 255, 255);border:none; border-radius:20px;color:rgb(93, 93, 94);"
                        "selection-background-color:rgb(242, 239, 249);selection-color:rgb(147, 115, 238);"
                        "padding: 20px;}");
    console->verticalScrollBar()->setStyleSheet(
                    "QScrollBar:vertical{background-color:none;width:12px;border-radius:6px}"
                    "QScrollBar::handle:vertical{background:rgb(187, 160, 250);border-radius:6px;min-height:20;}"
                    "QScrollBar::handle:vertical:hover{background:rgb(147, 115, 238);border-radius:6px;min-height:20;}"
                    "QScrollBar::add-line:vertical{height:0px;width:0px;}"
    ...
    console->horizontalScrollBar()->setStyleSheet(
                    "QScrollBar:horizontal{background:none;height:12px;border-radius:6px}"
                    "QScrollBar::handle:horizontal{background:rgb(187, 160, 250);border-radius:6px;min-width:20;}"
                    "QScrollBar::handle:horizontal:hover{background:rgb(147, 115, 238);border-radius:6px;min-width:20;}"
                    "QScrollBar::add-line:horizontal{height:0px;width:0px;}"
...
void IDELite::setDark()
{
    ...
    console->setStyleSheet("QPlainTextEdit{background-color:rgb(100, 100, 100);border:none; border-radius:20px;color:rgb(255, 255, 255);"
                        "selection-background-color:rgb(150,150,150);selection-color:rgb(75, 75, 75);"
                        "padding: 20px;}");
    console->verticalScrollBar()->setStyleSheet(
                    "QScrollBar:vertical{background-color:none;width:12px;border-radius:6px}"
...
void IDELite::on_replaceAction_triggered(){
    rmw.show();
}

// 由查询字符串初始化查询类
void IDELite::searchLineEdit_editingFinished()
{
    if(rmw.searchLine != ""){
        if(search != NULL){
            delete search;
...
// 开始查询
void IDELite::searchBeginPushButton_clicked()
{
...
// 上一个
void IDELite::previousSearchPushButton_clicked()
{
...
// 下一个
void IDELite::nextSearchPushButton_clicked()
{
...
void IDELite::replaceLineEdit_editingFinished()
{
    search->replaceString = rmw.replaceLine;
}

void IDELite::replacePushButton_clicked()
{
...
void IDELite::replaceAllPushButton_clicked()
{
